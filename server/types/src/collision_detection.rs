    ...
    pub fn new(cache: &mut CacheFileSystem) -> anyhow::Result<Self> {
        ...
        for index in map_indices.values() {
            ...
            for plane in 0..4 {
                ...
                let key = (plane as u8, index.get_x() as i16 / 64i16, index.get_y() as i16 / 64i16);
...
mod tests {
    ...
    pub fn test_collisions() {
        use pathfinding::prelude::{astar, Grid, absdiff};
        ...
        assert!(detector.is_traversable(start), "start is non-traversable; this is a bug");
